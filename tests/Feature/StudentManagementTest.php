...
class StudentManagementTest extends TestCase
{
    ...
    /**
     * An inactive academic session cannot be used when creating a student.
     */
    public function test_cannot_use_inactive_academic_session_for_student()
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);
        ...
        // create two sessions, make the first inactive
        $active = AcademicSession::factory()->create(['is_active' => true]);
        $inactive = AcademicSession::factory()->create(['is_active' => false]);
        ...
        $studentData = [
            ...
            'academic_year' => 'FY',
            'division_id' => $division->id,
            'academic_session_id' => $inactive->id,
            'admission_date' => '2025-06-01',
        ];
    ...
    /**
     * Activating a session should automatically deactivate any others.
     */
    public function test_only_one_academic_session_can_be_active()
    {
        $first = AcademicSession::factory()->create(['is_active' => true]);
        $second = AcademicSession::factory()->create(['is_active' => true]);

        $this->assertFalse($first->fresh()->is_active, 'first session should have been deactivated');
        $this->assertTrue($second->fresh()->is_active, 'second session remains active');
    }
